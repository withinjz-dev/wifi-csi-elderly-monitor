...
/*
 * EDGE_MODE = 1 : standalone operation. The board runs the whole pipeline --
 *                 CSI acquisition, feature extraction, motion and respiration
 *                 detection, adaptive calibration, state machine, alert -- and
 *                 needs no host attached. Serial output is one status line per
 *                 decision cycle, for a developer to watch, not to compute on.
 *
 * EDGE_MODE = 0 : legacy capture mode. Streams raw CSI_DATA rows for the host
 *                 Python reference implementation. Kept because the two are
 *                 cross-validated against each other on identical recordings.
 *
 * Only one may own the CSI callback, so this is a compile-time switch.
 */
#define EDGE_MODE 1
